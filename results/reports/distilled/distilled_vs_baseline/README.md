...
1. **Make a statement that elaborates at all.** The untrained model elaborates 111/244 (45.5%) of the slice; after distilled SFT that is 164/244 (67.2%). This is the bulk of the gain.
2. **Make the *intended* theorem.** Conditional on elaborating, the untrained model is right 11.7% of the time and the distilled model 22.6%. The one-directional `weaker_only` bucket is near-empty here (3 vs 2), so on this slice the failures are outright wrong statements rather than drifted ones.

Distilled data trains the model: it moves BEq+ from 5.3% to 15.2% and the statement-elaboration rate from 45.5% to 67.2%, both with overwhelming paired significance.
