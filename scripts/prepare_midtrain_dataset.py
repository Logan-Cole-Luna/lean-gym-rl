...
worth it -- so instead `scripts/validate_midtrain_corpus.py` type-checks a
...
Usage:
    python scripts/prepare_midtrain_dataset.py --out-dir data_3b/midtrain
...
def main() -> None:
    ...
    print(f"[midtrain] wrote {out}/sample.jsonl ({len(sample)}) -- run "
          f"scripts/validate_midtrain_corpus.py on it BEFORE training")
