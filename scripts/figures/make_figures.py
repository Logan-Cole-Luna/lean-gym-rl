...
def fig_greedy_two_panel(arms: dict[str, dict[int, list[dict]]], n: int,
                         panels=(("beq_plus", "BEq+ (%)"),
                                 ("typecheck", "compiles (%)"))):
    """The two-panel semantics/syntax comparison, from GREEDY evals.

    Same shape as fig_arm_trajectories_pass1, different source. That one reads
    passk_*.json, so an arm whose pass@k jobs have not run is simply absent from
    it; this one reads the eval_*.json every arm already has, which makes it the
    figure you can draw today and the denser one besides (every 10 steps rather
    than the four-point pass@k grid).

    It is a sound comparison for the same reason the pass@1 figure is: BOTH
    panels come off the SAME per_example records -- one decoder, one prompt set,
    only the verdict changes. What is NOT sound, and what the pass@1 figure
    exists to avoid, is reading greedy BEq+ against sampled compiling.
    """
    fig, axes = plt.subplots(1, 2, figsize=(12.4, 4.6))
    b = baseline()
    drew = False
    for ax, (metric, ylab) in zip(axes, panels):
        base = rate(b, metric, n)
        ends, ys_all = {}, []
        for arm, steps in sorted(arms.items()):
            ...
            xs = [0] + sorted(steps)
            ys = [base] + [rate(steps[k], metric, n) for k in sorted(steps)]
            ys_all += ys
            ax.plot(xs, ys, color=st["color"], marker=st["marker"],
                    markevery=list(range(1, len(xs))),
                    ls=st.get("linestyle", "-"), label=st["label"],
        ...
        ax.set_xlabel("GRPO step")
        ax.set_ylabel(ylab)
        pad = max(2.0, (max(ys_all) - min(ys_all)) * 0.22)
        ax.set_ylim(min(ys_all) - pad, max(ys_all) + pad)
        fs.finish(ax, end_labels=ends, legend_loc="lower left",
                  hline=(base, f"SFT {base:.1f}%"))
    return fig if drew else None
...
def fig_proxy_vs_semantic(arms, n, which=None):
    ...
    if tc:
        last = max(tc)
        fs.callout(ax, "type-check @ lr 1e-5, step 10 → %d:\nproxy +10pp, semantics −32pp" % last,
    ...
    for ax, (metric, ylab, kind) in zip(axes, panels):
        ...
        for arm, steps in sorted(arms.items()):
            ...
            ax.plot(*zip(*pts), color=st["color"], marker=st["marker"],
                    ...
                    markeredgecolor="white", markeredgewidth=0.7, zorder=3)
            ends[fs.end_label(st)] = (pts[-1][0], pts[-1][1], st["color"])
