...
(scripts/data/prepare_dataset.py) so there is no train/RL prompt mismatch.

Usage:
    python scripts/data/prepare_sft_dataset.py --n-train 4000 --n-val 200
"""
...
def main() -> None:
    ...
    if val_path.exists():
        ...
        print(f"WARNING: {val_path} not found -- cannot exclude the eval set from SFT training. "
              "Run scripts/data/prepare_dataset.py first.")
