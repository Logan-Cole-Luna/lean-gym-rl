...
Usage:
    python scripts/misc/build_rft_dataset.py \
...
def main() -> None:
    ...
    # `typecheck_not_beq` is the paper's exploitability claim in its pure form:
    # rollouts that elaborate but that BEq+ does NOT certify -- syntactically
    # valid, semantically wrong Lean. The plain `typecheck` filter dilutes that
    # contrast, because BEq+ implies type-check, so ~48% of what it accepts is
    # BEq+-certified anyway (measured on this scoring pass).
    ap.add_argument("--filter", default="beq_plus",
                    choices=["beq_plus", "typecheck", "typecheck_not_beq"])
    ...
    for pi, group in sorted(by_prompt.items()):
        ...
        if args.filter == "typecheck_not_beq":
            good = [g for g in group
                    if g["typecheck"] and not g["beq_plus"] and not g.get("error_kind")]
        else:
