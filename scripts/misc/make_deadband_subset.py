...
answer the question.
    ...
    python scripts/misc/make_deadband_subset.py \
...
Output is in `scripts/pool/score_rollouts.py`'s input format, so it re-scores with no
special casing.
"""
import argparse
import json
...
BANDS = ("dead", "no_typecheck", "one_direction", "beq_plus")


def band_of(r: dict) -> str:
    """Which rung of the gated ladder a scored rollout landed on.

    Module scope on purpose: report_deadband.py imports this, and the two must
    agree on what "dead band" means.
    """
...
def main() -> None:
    ...
    ap.add_argument("--band", choices=BANDS, default="dead")
    args = ap.parse_args()

    scored = [json.loads(l) for l in open(args.scored) if l.strip()]
    bands = [band_of(r) for r in scored]
    wanted = {(r["prompt_index"], r["sample_index"])
              for r, b in zip(scored, bands) if b == args.band}
