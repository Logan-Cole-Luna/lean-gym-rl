...
answers the Phase 0 question: **how often is the dead-band signal already free?**
    ...
    python scripts/report_deadband.py --scored data_3b/rollouts/deadband_sft3b-step93.scored.jsonl
...
def main() -> None:
    ...
    if "provable_alone" not in recs[0]:
        raise SystemExit(
            ...
            f"field. Re-score it with the current scripts/score_rollouts.py.")

    print(f"\n=== dead-band cascade instrumentation: {args.scored} ===")
    print(f"{n} rollouts\n")

    # Sanity: this pool is supposed to be dead band only.
    off = [r for r in recs if r.get("beq_plus") or not r.get("typecheck")]
    err = [r for r in recs if r.get("error_kind")]
    if off:
        print(f"!! {len(off)} rollouts are NOT dead band (beq_plus or no typecheck).")
